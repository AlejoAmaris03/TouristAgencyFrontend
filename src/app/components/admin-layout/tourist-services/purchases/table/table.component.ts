...
export class TableComponent {
  ...
  protected displayedColumns: string[] = ['id', 'serviceName', 'customerName', 'employeeName', 'dateOfSale', 'price'];
  private saleService = inject(SaleService);
  private authService = inject(AuthService);
  private sweetAlertService = inject(SweetAlertService);
  private toastrService = inject(ToastrNotificationService);
  private route = inject(ActivatedRoute);
  protected serviceId = Number(this.route.snapshot.paramMap.get("id"));
  ...
  public getPurchases(): void {
    this.saleService.getBuyersByServiceId(this.serviceId).subscribe({
      next: (purchases) => {
